...
import { BaseballLeague, BaseballTeamEntity } from "@/lib/models/baseball";
    ...
    return (
        <Card className="mb-4">
            <CardContent>
                ...
                <div className="flex flex-col mt-4 md:ml-4">
                    {!isLoading && (<>
                        ...
                        <p className="text-sm text-gray-600">
                            {[team?.currentRank ? `Current Rank: ${team?.currentRank}` : '-', team?.totalPoints ? `Total Points: ${team?.totalPoints}` : '-']
                                .filter(Boolean)
                                .join(' | ')}
                        </p>
                    </>)}
                ...
                <div className="flex flex-col mt-4 md:ml-4">
                    <a href={`https://fantasy.espn.com/baseball/team?leagueId=${leagueId}&teamId=${team?.id}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
